...
//    }
    ...
    fun rlp_decode_array() {
        ...
        println("decoded first int[]: ${decoded.testBytesList.first().contentToString()}")
    }

    @Test
    fun test() {
        val t: Type = ByteArray::class.java
        println("t: $t")

        println("it's${if (t is Class<*>) "" else " not"} class")
        println("it's${if (t is ParameterizedType) "" else " not"} ParameterizedType")
    }
}